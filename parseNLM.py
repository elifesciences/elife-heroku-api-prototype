...
def digest(soup):
	...
	for tag in abstract_soup:
		try:
			if(tag["abstract-type"] != None):
				# A tag attribute found, use this abstract
				abstract_node = tag
				break
		except KeyError:
				# No attribute, skip it
				pass
...
def get_custom_meta(soup):
	"""
	Find the kwd-group sections for further analysis to find
	subject_area, research_organism, and keywords
	"""
	custom_meta = None
	custom_meta = extract_nodes(soup, 'custom-meta')
	return custom_meta
...
def award_group_funding_source(soup):
	...
	for fg in funding_group_section[0]:
